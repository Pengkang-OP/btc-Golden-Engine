...
class Hash160Set:
    ...
    def load(
        ...
        if not os.path.exists(bin_path) or not os.path.exists(idx_path):
            raise FileNotFoundError(
                f"缺少 Hash160 数据文件。请先运行:\n"
                f"  python extract_utxo_hash160.py\n"
                f"需要: {bin_path}\n"
                f"      {idx_path}"
            )
        ...
        if not quiet:
            logger.info("已加载 %s 个 Hash160", f"{self._total:,}")
    ...
    def __contains__(self, h: bytes) -> bool:
        ...
        while lo < hi:
            mid = (lo + hi) // 2
            mid = mid - (mid % 20)  # 对齐到 20 字节边界
            chunk = mview[mid : mid + 20]
            if chunk < h:
                lo = mid + 20
            elif chunk > h:
                hi = mid
...
class XOnlySet:
    ...
    def load(
        ...
        if not os.path.exists(bin_path) or not os.path.exists(idx_path):
            raise FileNotFoundError(
                f"缺少 x-only pubkey 数据文件。请先运行:\n"
                f"  python extract_utxo_xonly.py\n"
                f"需要: {bin_path}\n"
                f"      {idx_path}"
            )
        ...
        if not quiet:
            ...
            if bloom_loaded:
                logger.info(
                    "Bloom Filter: %.0f MB (缓存)", self._bloom_m / 8 / 1_048_576
                )
            else:
                logger.info(
                    "Bloom Filter: %.0f MB (已构建)", self._bloom_m / 8 / 1_048_576
                )
    ...
    def _build_bloom(self, bin_path: str, quiet: bool = False) -> None:
        ...
        if not quiet:
            mb = byte_size / 1_048_576
            logger.info(
                "构建 Bloom Filter (%.0f MB, %s 条目)...", mb, f"{self._total:,}"
            )
    ...
    def _save_bloom(self, bin_path: str, bloom: bytearray) -> None:
        ...
        byte_size = len(bloom)
        header = bytearray()
        header.extend(_BLOOM_MAGIC)
        header.extend(struct.pack("<B", _BLOOM_VERSION))
        header.extend(struct.pack("<Q", self._total))
        header.extend(struct.pack("<Q", self._bloom_m))
        header.extend(bin_digest)
    ...
    def __contains__(self, h: bytes) -> bool:
        """检查 32 字节 x-only pubkey 是否在目标集中。"""
        rs = self.RECORD_SIZE
        if len(h) != rs:
            return False
